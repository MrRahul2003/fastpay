...
$bank_user_id = $_SESSION['bank_user_id'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!($_POST['myAmount']) || !($_POST['myPassword']) || !($_POST['cPassword'])) {
        ...
        if ($balance_available < $amount) {
            ...
            // Fetching the data of the bank sender which we had recently add in database -- bank_user from _pay_bank_details page
            $sql = "SELECT * FROM `bank_user` WHERE bank_user_no='$bank_user_id'";
            $result = mysqli_query($conn, $sql);

            $row = mysqli_fetch_assoc($result);
            $transaction_by = $row['bank_holder_name'];
            ...
            if ($password != $cpassword) {
                ...
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    ...
                    if (!$result) {
                        // echo "your data not has been inserted";
    ...
    <div class="container d-flex flex-row justify-content-around">
        ...
        <div class="col">
            <div class="card mb-4 rounded-3 shadow-sm border-primary">
                ...
                '<ul class="list-unstyled list-block text-center mt-3 mb-4">
                    <li class="list-block-item"><object data="../svg/svg-sender.svg" width="100" height="100"> </object></li>
                    <li class="list-block-item"><h2>' . $row['bank_holder_name'] . '</h2></li>
                    <li class="list-block-item">' . $row['IFSC_code'] . '</li>
                    <li class="list-block-item">' . $row['bank_holder_name'] . '@ok.' . $row['bank_name'] . '</li>
                </ul>
                <button type="button" class="w-100 btn btn-lg btn-primary">More Info</button>';
                ?>
                </div>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Payment to &nbsp;
                            ...
                            $row = mysqli_fetch_assoc($result);
                            echo '<h3>' . $row['bank_holder_name'] . '</h3>';
                            ?>
                        </h5>
                    ...
                    <div class="modal-body">
                        ...
                        <form action="/fastpay/payment/_pay_bank.php" method="POST">
